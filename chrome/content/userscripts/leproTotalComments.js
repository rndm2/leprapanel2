...
lpr2userscript_leproTotalComments.prototype = {
	...
	run: function(window, document, $) {
	    ...
	    var setViewMode = function (modeName) {
	        ...
	        if (modeName == 'author') {
	            var authorValue = document.getElementById('ltc-author-name').value;
	            document.location.hash = '#ltc-author-' + authorValue;
	        }
	        else {
	            document.location.hash = '#ltc-' + modeName;
	        }
	        //setCookie(viewModeCookie, modeName, document.location.pathname);
	    ...
	    var work = function () {
	        ...
	        if( document.location.pathname.search('comments') !== -1 ) {

	            createPanel();
	            appendAuthorsSearchLink();

	            if (document.location.hash.search('#ltc-') !== -1) {
	                var mode = document.location.hash.replace('#ltc-', '');
